#!/usr/bin/env node
/**
 * AnimeStream Database Builder v4
 * 
 * Combines v3's comprehensive IMDB title-matching with Cinemeta enrichment.
 * 
 * Features:
 * - Kitsu API - Full anime catalog (21,000+ entries) with genres
 * - Fribb/anime-lists - Primary IMDB/TMDB/TVDB mappings (~7,100 IMDB)
 * - IMDB Datasets - Title-matching fallback for remaining anime
 * - Cinemeta enrichment - Logos, backgrounds, cast from Stremio's metadata
 * - Season grouping - Merge seasons sharing same IMDB ID
 * 
 * Usage:
 *   node scripts/build-database-v4.js          # Full build
 *   node scripts/build-database-v4.js --test   # Test mode (500 items)
 *   node scripts/build-database-v4.js --skip-imdb  # Skip IMDB matching
 *   node scripts/build-database-v4.js --skip-cinemeta  # Skip Cinemeta enrichment
 */
...
const CONFIG = {
  ...
  cinemataDelayMs: 50, // 50ms between requests (faster than 100ms, still respectful)
  cinemataBatchSize: 5, // Process 5 in parallel
...
async function fetchCinemeta(imdbId, type = 'series') {
  try {
    ...
    return {
      ...
      cinemataName: meta.name || null, // English name from Cinemeta
...
async function enrichWithCinemeta(animeList) {
  ...
  // Process in batches for speed while being respectful
  for (let i = 0; i < animeList.length; i += CONFIG.cinemataBatchSize) {
    ...
    await Promise.all(batch.map(async (anime) => {
      ...
      if (cinemata) {
        ...
        // Merge genres (Cinemeta often has better English genre names)
        ...
        // Use Cinemeta's English name if our name is Japanese
  ...
  console.log(`      Cast: ${castCount}\n`);
  
  return { enriched, logos, backgrounds, cast: castCount };
}

// ============================================================
// JIKAN BROADCAST DATA ENRICHMENT
// ============================================================

const JIKAN_BASE_URL = 'https://api.jikan.moe/v4';
const JIKAN_RATE_LIMIT_MS = 400; // Jikan allows ~3 requests/second
...
async function fetchJikanBroadcast(malId) {
  try {
    ...
    return {
      broadcastDay: anime.broadcast?.day || null, // "Sundays", "Mondays", etc.
      broadcastTime: anime.broadcast?.time || null, // "23:00"
      broadcastTimezone: anime.broadcast?.timezone || null,
      broadcastString: anime.broadcast?.string || null, // Full string like "Sundays at 23:00 (JST)"
    };
  } catch (err) {
    return null;
  }
...
/**
 * Enrich ONGOING anime with broadcast schedule data from Jikan (MAL)
 * Only processes anime with status === 'ONGOING' and valid mal_id
 */
async function enrichWithJikanBroadcast(animeList) {
  console.log('\n📺 Fetching broadcast schedules from Jikan (MAL API)...');
  
  // Filter to only ONGOING anime with MAL IDs
  ...
  for (let i = 0; i < ongoingAnime.length; i++) {
    ...
    if (broadcastData && broadcastData.broadcastDay) {
      // Normalize day: "Sundays" -> "Sunday", "Mondays" -> "Monday"
  ...
  console.log(`\n\n   ✅ Jikan broadcast enrichment complete:`);
  console.log(`      Ongoing anime: ${ongoingAnime.length}`);
  console.log(`      With broadcast data: ${enriched} (${Math.round(enriched/ongoingAnime.length*100)}%)`);
  if (enriched > 0) {
    console.log(`      By day: ${JSON.stringify(weekdayStats)}`);
  }
  console.log('');
  
  return { total: ongoingAnime.length, enriched, weekdayStats };
}
...
function convertToStremioMeta(kitsuAnime, imdbId, malId) {
  ...
  return {
    ...
    logo: null, // Will be filled by Cinemeta
    cast: [], // Will be filled by Cinemeta
...
async function buildDatabase() {
  ...
  console.log('\n============================================================');
  console.log('       AnimeStream Database Builder v4');
  console.log('       (IMDB Title-Matching + Cinemeta Enrichment)');
  console.log('============================================================');
  ...
  // Step 7: Cinemeta enrichment (NEW in v4!)
  const cinemataStats = await enrichWithCinemeta(groupedAnime);
  
  // Step 8: Jikan broadcast enrichment for ONGOING anime (NEW in v4!)
  const jikanStats = await enrichWithJikanBroadcast(groupedAnime);
  
  // Step 9: Build filter options
  ...
  console.log(`   ✅ ${movies.length} movies (${upcomingMovies.length} upcoming, ${newReleaseMovies.length} new releases)\n`);
  ...
  const catalog = {
    ...
    version: '4.0',
    source: 'kitsu+fribb+imdb+cinemeta',
    stats: {
